...
export async function createDoctor(input: CreateDoctorInput) {
    try {
        ...
        //handle unique constraint violation (email alredy exists)
        if (error?.code === "P2002") {
            throw new Error("A doctor with this email alredy exists");
        }
...
export async function updateDoctor(input: UpdateDoctorInput) {
    try {
        ...
        // if email is changeing , check if the new email alredy exists
...
export async function getAvailableDoctors() {
    try {
        ...
        throw new Error("Failed to fetch available docotrs");
    }
}